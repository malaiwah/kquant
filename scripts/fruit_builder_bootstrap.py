...
_SOURCE_FINGERPRINT_PREFIX = b"qsrt-tracked-worktree-sha256-v1\0"
_BOOTSTRAP_CONTEXT_ENV = "QSRT_FRUIT_BUILDER_BOOTSTRAP_V5"
_CONTEXT_SCHEMA = "qsrt_fruit_builder_bootstrap_v5"
_RUNTIME_IDENTITY_SCHEMA = "qsrt_fruit_builder_external_oci_runtime_v1"
...
def snapshot_git_revision(
    ...
    *,
    ...
    revision = _digest(expected_revision, length=40, name="QSRT revision")
    expected_digest = _digest(expected_sha256, length=64, name="QSRT source SHA-256")
    ...
    snapshot = Path(tempfile.mkdtemp(prefix="qsrt-authenticated-source-", dir="/tmp"))
    ...
    try:
        ...
        if tree_digest.hexdigest() != expected_digest:
            raise ValueError(
                "authenticated Git revision does not match QSRT source SHA-256"
            )
        ...
        guard = snapshot / "scripts" / "qsrt_import_guard.py"
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Authenticate a QSRT Git snapshot and launch the Fruit QSRT builder"
    )
    parser.add_argument("--show-bootstrap-sha256", action="store_true")
    parser.add_argument("--qsrt-root", type=Path)
    parser.add_argument("--qsrt-revision")
    parser.add_argument("--qsrt-source-sha256")
...
def main() -> None:
    ...
    if None in (
        ...
        raise ValueError("production launch requires all external QSRT trust anchors")
    ...
    checkout = args.qsrt_root.resolve(strict=True)
    ...
    snapshot = snapshot_git_revision(
        checkout,
        expected_revision=args.qsrt_revision,
        expected_sha256=args.qsrt_source_sha256,
    )
    ...
    context = {
        ...
        "qsrt_revision": args.qsrt_revision,
        "qsrt_source_sha256": args.qsrt_source_sha256,
    ...
    runtime_root = Path(tempfile.mkdtemp(prefix="qsrt-fruit-runtime-", dir="/tmp"))
    private_directories = {
