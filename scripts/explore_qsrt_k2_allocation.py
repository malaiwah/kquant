...
def _functional_tile_pair_search_independent(
    ...
    *,
    ...
    fit_reference, fit_route_weights = _prepare_weighted_functional_target(
        source, inputs=fit_inputs, gates=gates[fit_mask]
    )
    baseline_fit_sse = _weighted_functional_sse(
        baseline_reconstruction,  # type: ignore[arg-type]
        inputs=fit_inputs,
    ...
    selected_name = min(scores, key=lambda name: float(scores[name]["fit"]["sse"]))
    baseline_name = "u0000/d0000"
    endpoint_names = (
        baseline_name,
        f"u{evaluated_count:04d}/d{evaluated_count:04d}",
    )
    selected_confirmation = float(scores[selected_name]["confirmation"]["sse"])
    endpoint_confirmation = min(
        float(scores[name]["confirmation"]["sse"]) for name in endpoint_names
    )
    return {
        "baseline": (
            {
        ...
        "baseline_reencode_evidence": {
            ...
            "interpretation": (
                "the final prefix-family n0000 candidate is authoritative; a "
                "single-map scale/path fit is retained only as screening evidence"
            ),
        },
        ...
        "confirmation_relative_to_p33": _relative(
            ...
            float(scores[baseline_name]["confirmation"]["sse"]),
        ),
