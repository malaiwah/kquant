...
if __name__ == "__main__":
    ...
    (
        ...
        _QSRT_IMPORT_SOURCE_ROOT,
        _QSRT_BOOTSTRAP_IDENTITY,
        _RUNTIME_QUALIFICATION_AUTHORITY_SHA256,
        _RATE_SWEEP_AUTHORITY_SHA256,
    ) = authenticate_production_builder(Path(__file__))
else:
    _QSRT_IMPORT_IDENTITY = None
    _QSRT_IMPORT_SOURCE_ROOT = Path(__file__).resolve().parents[1]
    _QSRT_BOOTSTRAP_IDENTITY = None
...
from qsrt.exl3_loader import exllamav3_source_identity, load_qsrt_encoder
from qsrt.fruit_calibration import (
    FruitCalibrationStore,
    fruit_calibration_authority,
)
from qsrt.fruit_qsrt import (
...
FRUIT_PUBLICATIONS = {
    "instruct": FruitPublicationSpec(
        ...
        introduction=(
            ...
            "it is not the 754B GLM-5.2 model. It is encoded in QSRT's canonical "
...
- QSRT encoder: [`local-inference-lab/qsrt#4`](https://github.com/local-inference-lab/qsrt/pull/4),
  encoded with QSRT revision `__QSRT_REVISION__`.
...
license. B12X and vLLM are Apache-2.0. QSRT is not redistributed in this
...
_RATE_SWEEP_SCHEMA = "qsrt_fruit_uniform_rate_sweep_v1"
_RATE_SWEEP_NAME = "evaluation/fruit-uniform-rate-sweep.json"
_RUNTIME_QUALIFICATION_SCHEMA = "qsrt_fruit_runtime_qualification_v1"
_RUNTIME_QUALIFICATION_NAME = "evaluation/fruit-runtime-qualification.json"
_RUNTIME_ARMS = ("bf16", "siq", "qsrt")
_RUNTIME_PATHS_SCHEMA = "qsrt_fruit_runtime_paths_v2"
...
_ENCODER_FINGERPRINT_SCHEMA = "qsrt_fruit_qsrt_encoder_source_v4"
_LEGACY_ENCODER_FINGERPRINT_SCHEMA = "qsrt_fruit_qsrt_encoder_source_v3"
...
def current_encoder_provenance(
    *,
    ...
    qsrt_root: Path | None = None,
    qsrt_identity: tuple[str, str] | None = None,
) -> dict[str, object]:
    if qsrt_root is not None and qsrt_identity is not None:
        raise ValueError(
            "QSRT root and authenticated identity are mutually exclusive"
        )
    if qsrt_root is not None:
        qsrt_checkout = qsrt_root.resolve(strict=True)
        qsrt_revision = _git_revision(qsrt_checkout)
        qsrt_source_sha256 = tracked_worktree_sha256(qsrt_checkout)
    elif qsrt_identity is not None:
        qsrt_revision, qsrt_source_sha256 = qsrt_identity
        if (
            len(qsrt_revision) != 40
            or len(qsrt_source_sha256) != 64
            or any(
                character not in "0123456789abcdef"
                for character in qsrt_revision + qsrt_source_sha256
            )
        ):
            raise ValueError("QSRT authenticated source identity is invalid")
    elif _QSRT_IMPORT_IDENTITY is not None:
        qsrt_revision, qsrt_source_sha256 = _QSRT_IMPORT_IDENTITY
    else:
        raise RuntimeError(
            "QSRT source identity requires the authenticated production bootstrap"
        )
    ...
    encoder: dict[str, object] = {
        ...
        "qsrt_source_sha256": qsrt_source_sha256,
        ...
        "encoding_runtime": (
            dict(_QSRT_BOOTSTRAP_IDENTITY)
            if _QSRT_BOOTSTRAP_IDENTITY is not None
            else None
        ),
...
def _producer_provenance(
    *,
    ...
    if _QSRT_BOOTSTRAP_IDENTITY is None:
        raise RuntimeError(
    ...
    provenance: dict[str, object] = {
        ...
        "bootstrap": dict(_QSRT_BOOTSTRAP_IDENTITY),
        "encoder": encoder,
        "runtime": runtime,
    }
...
def _validate_runtime_qualification(
    ...
    *,
    ...
    for arm in _RUNTIME_ARMS:
        ...
        for key in ("vllm_revision", "b12x_revision", "qsrt_revision"):
        ...
        if (
            ...
            or runtime["qsrt_revision"] != producer_encoder.get("qsrt_revision")
        ):
            raise ValueError(
        ...
        measured_runtime_identity = {
            ...
            "qsrt_revision": runtime["qsrt_revision"],
            "software": software,
        }
...
def _render_model_card(
    *,
    ...
    replacements = {
        ...
        "__QSRT_REVISION__": str(encoder["qsrt_revision"]),
...
def _validate_source_evidence(
    ...
    *,
    ...
    expected: dict[str, object] = {
        "kind": "qsrt-fruit-source-preflight",
        "version": 1,
        "status": "pass",
...
def _write_source_evidence_seal(
    ...
    *,
    ...
    envelope = {
        "schema": "qsrt_fruit_source_seal_v1",
...
def _read_source_evidence_seal(
    ...
    *,
    ...
    if (
        envelope.get("schema") != "qsrt_fruit_source_seal_v1"
...
def _write_calibration_evidence(
    ...
    _atomic_text(
        ...
        _canonical_json(
            {
                "schema": "qsrt_fruit_qsrt_calibration_reference_v1",
                "producer_fingerprint": producer["fingerprint"],
                "encoder_fingerprint": encoder["fingerprint"],
                **expected,
...
def _authenticate_base_model(
    ...
    *,
    ...
    return {
        "schema": "qsrt_fruit_bf16_base_v1",
...
def _strip_routed_experts(
    ...
    with tempfile.TemporaryDirectory(prefix="qsrt-base-shard-") as temporary:
