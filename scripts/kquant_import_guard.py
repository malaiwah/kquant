"""Authenticate the externally created source snapshot before KQuant imports."""
...
def _runtime_identity(value: object) -> dict[str, object]:
    ...
    if value.get("schema") != "kquant_fruit_builder_external_oci_runtime_v1":
...
def authenticate_production_builder(
    ...
    if already_imported:
        raise RuntimeError(
            "KQuant modules were imported before production bootstrap authentication: "
    ...
    revision = _digest(
        context.get("kquant_revision"), length=40, name="KQuant revision"
    )
    source_sha256 = _digest(
        context.get("kquant_source_sha256"),
        length=64,
        name="KQuant source SHA-256",
    )
    ...
    expected_guard = snapshot_root / "scripts" / "kquant_import_guard.py"
    if (
...
if _BOOTSTRAP_CONTEXT_ENV in os.environ:
    KQUANT_IMPORT_IDENTITY = None
    _KQUANT_IMPORT_SNAPSHOT = None
    KQUANT_IMPORT_SOURCE_ROOT = Path(__file__).resolve().parents[1]
else:
    (
