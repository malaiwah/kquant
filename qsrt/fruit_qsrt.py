...
from qsrt.candidate_hessian import weighted_effective_sample_size
...
from qsrt.qsrt import pack_trellis_edges, unpack_trellis_states
...
FRUIT_QSRT_SCHEMA = "qsrt_fruit_qsrt_pairs_v1"
...
FRUIT_QSRT_ATOM_SCHEMA = "qsrt_fruit_qsrt_atoms_v1"
...
def measure_fruit_uniform_rates(
    ...
    *,
    ...
    """Measure one expert at uniform K2/K3/K4 rates on sealed routed rows."""

    resolved_rates = tuple(rates)
    if not resolved_rates or any(
        type(bits) is not int or bits not in (2, 3, 4) for bits in resolved_rates
    ):
        raise ValueError("Fruit uniform rates must be a nonempty subset of K2/K3/K4")
    if len(set(resolved_rates)) != len(resolved_rates):
        raise ValueError("Fruit uniform rates must not contain duplicates")
    if device.type != "cuda":
        raise ValueError("Fruit uniform-rate measurement requires a CUDA device")
    if calibration.layer != layer or calibration.expert != expert:
        raise ValueError("Fruit calibration assignment does not match the expert")
    ...
    *,
    ...
    if quantizer_module is None:
        raise ValueError(
            ...
            "qsrt.exl3_loader.load_qsrt_encoder and install the SQG quantizer"
        )
