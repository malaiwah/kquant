...
from qsrt import fruit_calibration as calibration_module
...
def test_current_encoder_provenance_uses_process_lifetime_source_snapshot(
    ...
    qsrt_root = tmp_path / "qsrt-root"
    qsrt_source = qsrt_root / "qsrt" / "codec.py"
    qsrt_source.parent.mkdir(parents=True)
    qsrt_source.write_text("CODEBOOK = 1\n", encoding="utf-8")
    _commit_test_checkout(qsrt_root)
    monkeypatch.setattr(builder, "_QSRT_IMPORT_IDENTITY", None)
    ...
    anchored = builder.current_encoder_provenance(
        ...
        calibration=calibration,
        qsrt_identity=(
            str(before["qsrt_revision"]),
            str(before["qsrt_source_sha256"]),
        ),
    )
    source.write_text("CODEBOOK = 2\n", encoding="utf-8")
...
def test_model_card_uses_sealed_calibration_and_layer_evidence(
    ...
    producer = {
        "encoder": {"qsrt_revision": "qsrt-revision"},
        "runtime": {
...
def _runtime_qualification_fixture(
    ...
    producer = {
        "schema": "qsrt_fruit_qsrt_producer_v1",
        "encoder": {
            "fingerprint": "a" * 64,
            "qsrt_revision": revisions["qsrt_revision"],
        },
        "runtime": {
            "fingerprint": "b" * 64,
...
def test_candidate_seal_precedes_and_binds_runtime_qualification(
    ...
    assert complete["schema"] == "qsrt_complete_v3"
...
def test_rate_sweep_validation_binds_build_provenance(tmp_path: Path) -> None:
    ...
    repinned_encoder["qsrt_revision"] = "8" * 40
    ...
    changed_encoder["qsrt_revision"] = "9" * 40
