...
| --- | --- | --- |
| One- or two-bit tile-local K2 codebook menu | Yes | Rejected for the first pure-K2 format. On 28 experts across seven layers, the four-law fit-selected menu regressed 0.080% pooled on the separate 128K corpus. Confirmation gating recovered only 0.072%, and the conservative normal/Q33 gate recovered 0.066%; neither justifies tile metadata, alternate LUTs, or kernel branches. Use the single normal T12 staircase. |
